...
void GraphicsItemPlanet::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if(m_owner == UNASSIGN)
    {
        icon = QIcon("/models/planetGrey.svg").pixmap(QSize(50, 50));

    }
    else if (m_owner == PLAYER1)
    {
        icon = QIcon("/models/planetGreen.svg").pixmap(QSize(50, 50));
    }
    else if (m_owner == PLAYER2)
    {
        icon = QIcon("/models/planetRed.svg").pixmap(QSize(50, 50));
    }
